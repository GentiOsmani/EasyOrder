...
// In packaged: __dirname = <install>/resources/app.asar/out/main/
//              extraResources land in <install>/resources/
...
if (process.platform === 'win32') {
  app.setAppUserModelId('com.easyorder.app')
}
...
const currentDeviceBindings = db.get('deviceBindings').value() || {}
...
const nextDeviceBindings = {
  staffTerminalUserId: Number.isInteger(currentDeviceBindings.staffTerminalUserId) ? currentDeviceBindings.staffTerminalUserId : null,
  lockedAt: currentDeviceBindings.lockedAt || null
}
...
const migrationChanged =
  ...
  currentDeviceBindings.staffTerminalUserId === undefined ||
  currentDeviceBindings.lockedAt === undefined
...
if (migrationChanged) {
  db
    ...
    .set('deviceBindings', nextDeviceBindings)
    .write()
}
...
function getDeviceBindings() {
  const bindings = db.get('deviceBindings').value() || {}
  return {
    staffTerminalUserId: Number.isInteger(bindings.staffTerminalUserId) ? bindings.staffTerminalUserId : null,
    lockedAt: bindings.lockedAt || null
  }
}

function setStaffTerminalBinding(userId) {
  db
    .set('deviceBindings.staffTerminalUserId', userId)
    .set('deviceBindings.lockedAt', new Date().toISOString())
    .write()
}

function clearStaffTerminalBinding() {
  db
    .set('deviceBindings.staffTerminalUserId', null)
    .set('deviceBindings.lockedAt', null)
    .write()
}
...
// Serve built renderer (production only — dev uses Vite dev server)
if (fs.existsSync(rendererDir)) {
  expressApp.use(express.static(rendererDir))
}

// Serve client phone menu — v4 REBUILD 2026-04-19 04:55
...
expressApp.get('/menu/:tableId', (req, res) => {
  console.log('[SERVER] Serving menu HTML with NO-CACHE headers')
  res.set('Cache-Control', 'no-store, no-cache, must-revalidate')
  res.set('Pragma', 'no-cache')
  res.set('Expires', '0')
...
expressApp.post('/api/auth/login', (req, res) => {
  ...
  if (!roleAllowedForAppMode(user.role, appMode)) {
  ...
  if (appMode === 'staff') {
    ...
    if (bindings.staffTerminalUserId && bindings.staffTerminalUserId !== user.id) {
      const lockedUser = db.get('users').find({ id: bindings.staffTerminalUserId }).value()
      return res.status(423).json({
        error: `This terminal is locked to ${lockedUser?.username || 'another account'}`
      })
    }
    if (!bindings.staffTerminalUserId) {
      setStaffTerminalBinding(user.id)
    }
  }
...
expressApp.get('/api/auth/device-binding', (req, res) => {
  const appMode = normalizeAppMode(req.query.appMode || APP_MODE)
  if (appMode !== 'staff') {
    return res.json({ locked: false, appMode })
  }
  ...
  const user = db.get('users').find({ id: bindings.staffTerminalUserId }).value()
  if (!user || !roleAllowedForAppMode(user.role, 'staff')) {
...
function withAppModeQuery(urlValue) {
  try {
    const url = new URL(urlValue)
    url.searchParams.set('appMode', APP_MODE)
    return url.toString()
  } catch {
    return urlValue
  }
...
function createWindow() {
  ...
  if (isDev && process.env.ELECTRON_RENDERER_URL) {
    mainWindow.loadURL(withAppModeQuery(process.env.ELECTRON_RENDERER_URL))
    mainWindow.webContents.openDevTools()
  } else {
    const tryLoad = () => {
      if (serverReady) mainWindow.loadURL(withAppModeQuery('http://localhost:3001'))
