// ─── Config ─────────────────────────────────────────────────────────────────
// Reads backend URL and table ID from query params:
//   ?server=https://your-app.onrender.com&table=5
// Falls back to same-origin (for local Express serving) if no server param.
const params = new URLSearchParams(location.search)
const SERVER_URL = (params.get('server') || '').replace(/\/+$/, '')
const tableId = parseInt(params.get('table') || location.pathname.split('/').pop()) || 1
...
function pollOrderStatus() {
  ...
  statusPollInterval = setInterval(async () => {
    try {
      const r = await fetch(api(`/api/orders/status/${currentOrderId}`))
...
// ─── Init ────────────────────────────────────────────────────────────────────
async function init() {
  // If no server param and we're on GitHub Pages (not served by Express), show error
  if (!SERVER_URL && !location.pathname.startsWith('/menu/')) {
  ...
  try {
    const ctxRes = await fetch(api(`/api/client/context/${tableId}`))
    ...
    document.getElementById('menu-list').innerHTML =
      '<p style="text-align:center;color:#ef4444;padding:60px 16px;font-size:15px">Could not load menu. Check your connection and <a href="" style="color:#f59e0b">tap to retry</a>.</p>'
  }
}
